...
use super::trace::{FunctionCall, Opid};
...
pub struct PTrace {
    pub ops: Vec<PTraceOp>,
}

impl PTrace {
    #[allow(dead_code)]
    pub fn new() -> Self {
        Self { ops: Vec::with_capacity(100_000) }
    }

    pub fn record(
        &mut self,
        content: PTraceOpContent,
        parent: Option<Opid>,
    ...
    AdapterT::Vertex: VertexT + 'vertex,
{
    Box::new(make_iter_with_perf_span(result_iter, move |result, d| {
        adapter_tap.tracer.borrow_mut().record(PTraceOpContent::ProduceQueryResult, None, Some(d));
        result
    }))
}
    ...
    fn resolve_starting_vertices(
        ...
        let x = make_iter_with_perf_span(inner_iter, move |v, d| {
            tracer_ref_1.borrow_mut().record(
                PTraceOpContent::YieldFrom(PYieldValue::ResolveStartingVertices),
                Some(call_opid),
                Some(d),
            );
            v
