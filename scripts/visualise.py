        ...
        while i < len(operations):
            ...
            if op.type == "YieldInto":
                ...
                if op.type == "YieldFrom(ResolveNeighborsInner)":
                    ...
                    assert operations[idx+1].type in ("YieldInto", "OutputIteratorExhausted", "AdvanceInputIterator", "ProduceQueryResult")
                    if operations[idx+1].type == "YieldInto":
                        ...
                        assert operations[idx+2].type in ("YieldInto", "AdvanceInputIterator")
                ...
                if op.type in ("YieldFrom(ResolveProperty)", "YieldFrom(ResolveCoercion)"):
                    if operations[i+1].type == "YieldInto":
                        ...
                        if repeat_warnings or not warned_starting_vertices:
                            print(f"Warning ({i+1}L): Non-ResolveStartingVertices YieldFrom is not followed by YieldInto or an AdvanceInputIterator.")
...
for entry in os.scandir(R"C:\Users\josep\dev\gsoc\cargo\trustfall\scripts\outputs"):
    if not entry.is_file():
        continue
    if entry.name not in ("type_requires_more_const_generic_params.ron.ptrace.txt",):
        continue
    # if entry.name < "method_parameter_count_changed.ron.ptrace.txt":
    #     continue

    print(f"=== {entry.name.split('.')[0]} === ")
    yield_intos, yield_froms, advance_iterators, operations, total_time = calc_yields(entry.path)
    ...
    for yf, yi in yield_froms.items():
        if yi == None:
            ...
            self_times[operations[int(yf)-1].id] = operations[int(yf)-1].duration - operations[int(yi)-1].duration
    ...
    for yi, yfs in yield_intos.items():
        ...
        if raw_duration < (duration - np.timedelta64(10, 'ns')):
            ...
            print(sorted(yfs))
            print(f"Raw duration for {yi} less than calc duration: ({raw_duration} < {duration})")
