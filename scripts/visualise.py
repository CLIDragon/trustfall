...
def calc_yields(filename):
    ...
    pbar = tqdm(total=len(operations))
    try:
        i = 0
...
for entry in os.scandir(R"C:\Users\josep\dev\gsoc\cargo\trustfall\scripts\outputs_serde"):
    ...
    yield_intos, yield_froms, _, operations, total_time = calc_yields(entry.path)
    ...
    # YF(id) : self._time
    self_times = {}
    for yf, yi in tqdm(yield_froms.items()):
        if yi == None:
    ...
    for yf, time in tqdm(self_times.items()):
    ...
    for parent, times in tqdm(parents.items()):
    ...
    for parent, times in sorted(n_parents.items(), key=lambda x: -np.array(x[1]).sum()):
        ...
        print(f"mean: {mean} median: {np.median(times)}".replace(" nanoseconds", "ns"), end="\n")

    # print(n_parents["ResolveProperty(Vid(4), \"ImportablePath\", \"public_api\")"])
