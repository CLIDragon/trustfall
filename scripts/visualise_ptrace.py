...
class TraceReplay:
    ...
    def _print_debug_info(self, op, indent=True):
        ...
        print(
            f"{ind} Operation: {op}",
            f"{ind} Stack: {self._callstack}",
            f"{ind} Index: {self._cursor}",
            f"{ind} Last Yield: {self._last_yield}",
        )
    ...
    def ascend(self):
        ...
        if len(parents) != 1:
            print(
                "Invalid number of ancestors when walking up the tree. If this is the final operation, you can safely ignore this error."
            )
